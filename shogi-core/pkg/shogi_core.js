...
export class Board {
     ...
     * @returns {boolean}
     */
    set_piece_by_coords(row, col, piece, player) {
        const ret = wasm.board_set_piece_by_coords(this.__wbg_ptr, row, col, piece, player);
        return ret !== 0;
    }
    /**
     * @param {Position} position
    ...
    clear_square(position) {
        _assertClass(position, Position);
        var ptr0 = position.__destroy_into_raw();
        const ret = wasm.board_clear_square(this.__wbg_ptr, ptr0);
        return ret !== 0;
    }
    /**
     * @param {number} row
     ...
     * @returns {boolean}
     */
    clear_square_by_coords(row, col) {
        const ret = wasm.board_clear_square_by_coords(this.__wbg_ptr, row, col);
        return ret !== 0;
    }
    clear_board() {
        wasm.board_clear_board(this.__wbg_ptr);
    }
    /**
     * @param {Player} player
     */
    set_current_player(player) {
        wasm.board_set_current_player(this.__wbg_ptr, player);
    }
    reset_to_initial_position() {
        wasm.board_reset_to_initial_position(this.__wbg_ptr);
    }
    /**
     * @param {Player} player
     * @param {Piece} piece
     ...
     * @returns {boolean}
     */
    set_captured_piece_count(player, piece, count) {
        const ret = wasm.board_set_captured_piece_count(this.__wbg_ptr, player, piece, count);
        return ret !== 0;
    }
    /**
     * @param {Player} player
     */
    clear_captured_pieces(player) {
        wasm.board_clear_captured_pieces(this.__wbg_ptr, player);
    }
    /**
     * @returns {Int32Array}
     */
    get_all_captured_pieces() {
        const ret = wasm.board_get_all_captured_pieces(this.__wbg_ptr);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('shogi_core_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
